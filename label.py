...
class ImageClassifier(tk.Frame):
    def __init__(self, parent, src, *args, **kwargs):
        ...
        normScale = tk.Scale(self.window, from_=21000, to_=22999, length=380, orient=tk.VERTICAL, variable=self.scale, command=lambda v: self.change_overlay(scale=True))
        normScale.grid(row=2, column=4, padx=2, pady=2)
        normMaxScale = tk.Scale(self.window, from_=23000, to_=27000, length=380, orient=tk.VERTICAL, variable=self.scaleMax, command=lambda v: self.change_overlay(scale=True))
